...
// if(!empty($POST_['mode'])){} 
  ...
  //２．データ取得SQL作成
  $stmt = $pdo->prepare("SELECT * FROM gs_yt_table WHERE unread = 0 AND loginID =  $loginID ");
  $status = $stmt->execute();
  ...
  if ($status==false) {
    ...
    //FETCH_ASSOC=http://php.net/manual/ja/pdostatement.fetch.php
      ...
      while ($result = $stmt->fetch(PDO::FETCH_ASSOC)) {
        ...
        $view .= "{$deleteid}";
        $view .= '&unread=1';
        $view .= '">削除する</a></td></tr><tr><td><a href="chatroom/Client/chat.php?chatid=';
        $view .= "{$chatid}";
        $view .= '">チャットする</a></td></tr></table></div>';
    }
  }  

  
  //4．CSVデータを作る
  $stmt2 = $pdo->prepare("SELECT * FROM gs_yt_table WHERE unread = 0 AND loginID =  $loginID ");
