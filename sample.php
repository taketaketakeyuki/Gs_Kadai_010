...
//　0.if文で動作をかき、ボタンが押されないと値が入らないmodeに関するnotceを消す
//emptyメソッド。からかどうかを判定する。今回は「！」つけて”空でなければ”を判定する
// if(){}
// if(!empty($POST_['mode'])){} 
  ...
  //２．データ取得SQL作成
  $stmt = $pdo->prepare("SELECT * FROM gs_yt_table WHERE unread = 0 AND loginID =  2 ");
  $status = $stmt->execute();
  ...
  //4．CSVデータを作る
  $stmt2 = $pdo->prepare("SELECT * FROM gs_yt_table WHERE unread = 0 AND loginID =  2 ");
...
<head>
    <meta charset="UTF-8">
    <title>Youtube一覧シートPHP版(非ログイン)</title>
...
<body>
    ...
    <header>
        ...
        <nav class="navbar navbar-default">
            ...
            <div class="container-fluid">
                <div class="navbar-header">Youtube一覧シートPHP版（非ログイン）</div>
    ...
    <!-- Main[Start] -->
        ...
        <div class="jumbotron">
        ようこそ！新規登録は<a href="SignUp.php">こちら</a>、ログインは<a href="login.php">こちら</a>
        </div>

    <!-- Main[End] -->

    <!--CSV出力ボタン-->
    <!-- <a href="data/data.csv">CSV一覧をDLする</a> -->
